...
class The(BotModule):
    ...
    def __init__(self, parent, default_module_node):
        super(The, self).__init__(parent, default_module_node)
    ...
    def call_handle(self):
        ...
        if len(splited_msg) < 2 or sender in splited_msg[1]:
            self.c.privmsg(target, "" + sender.nick + " se prépare un e-thé bien chaud. Enfin prêt pour la journée !")
        else:
            self.c.privmsg(target, "" + sender.nick + " offre un e-thé bien chaud à " + splited_msg[1] + ". Préparé avec amour !")
